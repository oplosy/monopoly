...
- **Settings.** `dealcity.volume` (0–1, default 0.6) and `dealcity.muted` (`'1'` when muted) in `localStorage`, read and written without ever throwing; junk reads as the defaults. Moving the slider unmutes; unmuting at volume 0 brings it back to 0.6.
