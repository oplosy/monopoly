...
- **Original art only.** Everything is drawn in code as SVG or CSS, like the cards. No UNO, Hasbro or Monopoly assets and no traced reference art. Sound comes from CC0 packs (§8). *Amended by Plan 9 (`2026-09-25-scene-art-design.md`): the scene layers are raster art generated by the user; cards, avatars, HUD and UI stay in code.*
...
4. **Plan 9: Painted scene and ambient life** (branch `feat/scene-art`), added 2026-09-25 by its own addendum spec, `2026-09-25-scene-art-design.md`, which overrides §4.1 (ground and table plane), §4.5 (props) and extends §6, §8 and §9.4.
